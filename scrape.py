...
def extract_course_id(course_url):
    """Extract course ID from 'tn' parameter in URL."""
...
def fetch_ajax_review_info(product_id):
    ...
    '''
    params = {
        "container": "abtest-course-info-container",
        "handle": "catalog_product_view_type_simple,catalog_product_view",
        "id": course_id
    }
    '''

    try:
        #response = requests.get(ajax_url, headers=HEADERS, params=params, timeout=10)
...
def extract_course_metadata(url):
    ...
    try:
        ...
        #review_tag = soup.find("div", class_="bv_numReviews_text")
        #num_reviews = int(re.search(r"(\d+)", review_tag.get_text(strip=True).replace(",", "")).group(1)) if review_tag else 0
